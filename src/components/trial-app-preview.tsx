...
// ==========================================================================
// Association Dashboard — the account model
// ==========================================================================
function AssociationDashboard() {
  ...
  return (
    <div className="mb-4 rounded-2xl border border-primary/25 bg-gradient-to-br from-primary/5 via-card to-card p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
              <Mail className="size-3.5" /> Signed in as{" "}
        ...
        <button
          ...
          className="rounded-full border border-border/70 bg-background px-3 py-1 text-[11px] font-semibold text-foreground/80 transition hover:border-primary/40 hover:bg-muted"
        >
      ...
      {expanded && (
        <>
          {/* Past trials — finance kept, participant data purged */}
          <div className="mt-5">
            <div className="flex items-center justify-between gap-3">
              <p className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-[0.18em] text-primary">
                <History className="size-3.5" /> Past trials
              </p>
              <p className="text-[11px] italic text-muted-foreground">
                Finance kept · participant data removed 1 month after each
                trial finishes.
              </p>
            </div>
            <ul className="mt-3 space-y-2">
              {SAMPLE_PAST_TRIALS.map((t) => (
                <li
                  key={t.name}
                  className="grid gap-2 rounded-xl bg-background px-4 py-3 text-sm sm:grid-cols-[1fr_auto_auto_auto] sm:items-center sm:gap-6"
                >
                  <div>
                    <p className="font-semibold text-foreground">{t.name}</p>
                    <p className="mt-0.5 text-[11px] text-muted-foreground">
                      {t.ageGroups.join(", ")} · {t.finalisedOn}
                    </p>
                  </div>
          ...
          {showNewTrial && (
            <div className="mt-3 rounded-xl border border-dashed border-primary/40 bg-background p-4 text-xs">
              <p className="font-semibold text-foreground">
                New trial — same login, fresh setup
              </p>
              <p className="mt-1 text-muted-foreground">
                Clicking Create takes you back to Step 1 with a blank form —
                different age groups, different sessions, different venue,
                all under the same {SAMPLE_ACCOUNT.email} account. Past
                trials stay in the history above.
              </p>
              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  type="button"
                  className="inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-[11px] font-semibold text-primary-foreground transition hover:scale-[1.03]"
                >
                  Create new trial
                </button>
                <button
                  type="button"
                  onClick={() => setShowNewTrial(false)}
                  className="rounded-full border border-border/70 bg-background px-4 py-2 text-[11px] font-semibold text-foreground/80 transition hover:border-primary/40 hover:bg-muted"
                >
                  Cancel
                </button>
              </div>
